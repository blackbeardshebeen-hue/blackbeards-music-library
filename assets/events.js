...
function makeICS(ev){
  ...
  return ['BEGIN:VCALENDAR','VERSION:2.0','PRODID:-//Blackbeards Music Library//EN','BEGIN:VEVENT','DTSTART;VALUE=DATE:'+dateStr,'DTEND;VALUE=DATE:'+nextStr,'SUMMARY:'+ev.title,'LOCATION:'+(ev.venue||'')+(ev.address?', '+ev.address:''),'DESCRIPTION:'+ev.description+(ev.link?'\n\n'+ev.link:''),'END:VEVENT','END:VCALENDAR'].join('\r\n');
}
...
function gcalURL(ev){
  ...
  return base+'&text='+encodeURIComponent(ev.title)+'&dates='+ds+'/'+ns+'&details='+encodeURIComponent(ev.description+(ev.link?'\n\nMore info: '+ev.link:''))+'&location='+encodeURIComponent((ev.venue||'')+(ev.address?', '+ev.address:''));
}

function shareEvent(ev){
  var text=ev.title+' — '+ev.venue+', '+(ev.city||'Bay Area')+' ('+ev.date+')';
  var url='https://blackbeards-music-library.pages.dev/events';
  if(navigator.share){navigator.share({title:ev.title,text:text,url:url}).catch(function(){});}
  else{navigator.clipboard.writeText(text+' '+url).then(function(){var btn=document.querySelector('[data-sharetitle="'+CSS.escape(ev.title)+'"]');if(btn){var orig=btn.textContent;btn.textContent='Copied!';setTimeout(function(){btn.textContent=orig;},1800);}}).catch(function(){});}
}
...
function cardHTML(ev){
  ...
  var calDropdown=!isPast?'<div class="cal-dropdown"><button class="event-action-btn cal-trigger" aria-label="Add to calendar">📅 Save</button><div class="cal-menu"><a href="'+gcalURL(ev)+'" target="_blank" rel="noopener">Google Calendar</a><a href="#" class="ics-download" data-title="'+encodeURIComponent(ev.title)+'">Apple / iCal</a></div></div>':'';
  ...
  return '<div class="event-card anim-entry'+(isPast?' past':'')+'" data-evtitle="'+safeTitle+'" data-city="'+ev.city+'">'+
    '<div class="event-date-block"><div class="event-month">'+MONTHS[d.getMonth()]+'</div><div class="event-day">'+d.getDate()+'</div><div class="event-weekday">'+DAYS[d.getDay()]+'</div></div>'+
    '<div class="event-info"><div class="event-title">'+ev.title+'</div><div class="event-meta">'+meta+'</div><div class="event-desc">'+ev.description+'</div><div class="event-actions">'+calDropdown+shareBtn+infoLink+'</div></div>'+
...
function wireCards(){
  root.querySelectorAll('.ics-download').forEach(function(a){
    a.addEventListener('click',function(e){
      ...
      var tmp=document.createElement('a');tmp.href=url;tmp.download=title.replace(/[^a-z0-9]/gi,'-').toLowerCase()+'.ics';tmp.click();URL.revokeObjectURL(url);
    });
  });
  ...
  document.addEventListener('click',function(){document.querySelectorAll('.cal-menu.open').forEach(function(m){m.classList.remove('open');});});
  requestAnimationFrame(function(){
    root.querySelectorAll('.event-card.anim-entry').forEach(function(c,i){setTimeout(function(){c.classList.add('visible');},i*40);});
  });
}
...
function render(){
  ...
  if(!filtered.length){root.innerHTML='<div class="events-empty"><span class="skull">☠️</span><p>No shows match those filters.<br>Try broadening your search.</p></div>';return;}
  var html='';
  if(activeCity!=='all'||searchQuery){
    ...
    CITY_ORDER.forEach(function(city){
      ...
      html+='<div class="geo-section" data-city="'+city+'">'+
        '<div class="geo-header"><div class="geo-title">'+(CITY_LABELS[city]||city)+'</div><span class="geo-count">'+evs.length+'</span><div class="geo-line"></div></div>'+
